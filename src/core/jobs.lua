...
function lib.sendItemToInventory(...)
    ...
    return inventory:get(args.item, count, args.location, args.slot)
end
...
function lib.listenInventory(...)
    ...
    while (ok and item) do
        local request = inventory:put(item, inv, n)
        if not request.ok then
            return request
        end
        n = n + 1
...
function lib.keepMinItemInSlot(...)
    ...
    if ok then
        if not item then
            local req = inventory:get(itName, min, inv, slot)
            if not req.ok then
                return req
            else
                utils.updateClients()
        ...
        elseif item.count < min then
            ...
            local req = inventory:get(itName, diff, inv, slot)
            if not req.ok then
                return req
            else
                utils.updateClients()
