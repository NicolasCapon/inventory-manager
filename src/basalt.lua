...
baa["left"]=colors[bc("borderLeft",__b)]end;if(bc("borderTop",__b)~=nil)then
baa["top"]=colors[bc("borderTop",__b)]end;if(bc("borderRight",__b)~=nil)then
baa["right"]=colors[bc("borderRight",__b)]end
if
(bc("borderBottom",__b)~=nil)then baa["bottom"]=colors[bc("borderBottom",__b)]end;if(bc("borderColor",__b)~=nil)then
...
tostring(bbb).. (#dab~=abb and", "or"")end;daa.debugLabel:setText(_bb)
for abb,bbb in
...
return cdb(_cc(bcc or cc(),acc))end end;setmetatable(caa,d_a)return caa end end;aa["objects"]={}
aa["objects"]["Label"]=function(...)local ab=da("Object")
local bb=da("utils")local cb=bb.getValueFromXML;local db=bb.createText;local _c=da("tHex")
local ac=da("bigfont")
return
function(bc)local cc=ab(bc)local dc="Label"cc:setZIndex(3)local _d=true
cc:setValue("Label")cc.width=5;local ad="left"local bd="top"local cd=0;local dd,__a=false,false
local a_a={getType=function(b_a)return dc end,setText=function(b_a,c_a)
c_a=tostring(c_a)cc:setValue(c_a)
if(_d)then local d_a=b_a.parent:getOffset()
if
(
c_a:len()+b_a:getX()>b_a.parent:getWidth()+d_a)then
local _aa=b_a.parent:getWidth()+d_a-b_a:getX()cc.setSize(b_a,_aa,#db(c_a,_aa))else
...
c_a[_aa]={term.nativePaletteColor(_aa)}end end;if(dc.palette~=nil)then
for d_a,_aa in pairs(dc.palette)do c_a[2 ^d_a]=_aa end end;if(dc[b_a]~=nil)and
(dc[b_a].palette~=nil)then
for d_a,_aa in pairs(dc[b_a].palette)do c_a[2 ^d_a]=_aa end end;return c_a end end
...
if(ad.init(ada))then
ada.bgColor=ada.parent:getTheme("TextfieldBG")ada.fgColor=ada.parent:getTheme("TextfieldText")end end}return setmetatable(_da,ad)end end;aa["libraries"]={}
